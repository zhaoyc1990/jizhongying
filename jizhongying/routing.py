...
from server.consumers import connect_log, disconnect_log, save_post
...
channel_routing = [
    ...
    route("websocket.connect", connect_log, path=r'^/serverlog/(?P<abcd>[^/]+)/stream/$'),

    # Called when the client closes the socket
    route("websocket.disconnect", disconnect_log, path=r'^/serverlog/(?P<abcd>[^/]+)/stream/$'),

    # Called when the client sends message on the WebSocket
    route("websocket.receive", save_post, path=r'^/serverlog/(?P<abcd>[^/]+)/stream/$'),
